...
def register():
    if request.method == 'POST':
        ...
        if not is_password_complex(password):
            flash("Password must contain 8 characters, 1 uppercase letter, and 1 number.")
...
def index():
    ...
    try:
        ...
        app.logger.error(f"Task load error: {err}")
...
def add_task():
    ...
    try:
        cursor.execute(
            """INSERT INTO tasks (name, status, priority, user_id)
               VALUES (%s, %s, %s, %s)""",
